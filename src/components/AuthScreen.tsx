...
// Friendlier copy for the noisy Supabase auth errors users actually hit.
...
export default function AuthScreen({ initialMode = "login" }: { initialMode?: AuthMode }) {
  ...
  function switchMode(next: AuthMode) {
    ...
    setError(null);
    setInfo(null);
    // Keep the URL honest so refresh / share lands on the right form.
    const targetPath = next === "signup" ? "/signup" : "/login";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // when it's on, we land here with no session, so tell the user to check
      // their inbox instead of silently sitting on the form.
      if (mode === "signup" && !useAuth.getState().user) {
          ...
          <div className="mx-auto flex w-full max-w-[420px] flex-1 flex-col justify-center pt-6 lg:pt-0">
            ...
            >
              ...
              {info && !error && (
                <div
                  role="status"
                  className="rounded-2xl bg-teal/15 px-4 py-2.5 text-xs text-teal ring-1 ring-teal/30"
                >
                  {info}
                </div>
              )}
